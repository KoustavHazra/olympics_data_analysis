...
st.sidebar.image('assets/olympics_logo.png')
...
if user_menu == 'Country-wise Analysis':
    ...
    country_list.sort()
    selected_country_list = st.sidebar.selectbox('Select a country', country_list)

    country_df = helper.year_wise_medal_tally(df, selected_country_list)
    fig = px.line(country_df, x='Year', y='Medal')
    st.title(f'Medal tally of {selected_country_list} over the years')
    st.plotly_chart(fig)

    pt = helper.country_sport_heatmap(df, selected_country_list)
    if not pt.empty:
        st.title(selected_country_list + "'s performance in different sports")
        fig, axis = plt.subplots(figsize=(20, 20))
        ax = sns.heatmap(pt, annot=True)
        st.pyplot(fig)

    st.title(f'Top 10 athletes of {selected_country_list}')
    top10_df = helper.most_successful_country_wise(df, selected_country_list)
    st.table(top10_df)
...
if user_menu == 'Athlete-wise Analysis':
    ...
    st.title('Distribution of Age vs Medals')
    ...
    st.title('Distribution of Age vs Sports[wrt Gold Medalist]')
    fig.update_layout(autosize=False, width=900, height=600)
    st.plotly_chart(fig)
